...
enum IndicatorState { STATE_STOPPED_OK, STATE_PLAYING, STATE_WIFI_PROBLEM, STATE_BUFFERING_WARN };
...
void updateDisplay() {
    ...
    if (currentAudioMode == MODE_WIFI_RADIO) {
        ...
        tft.setCursor(5, SCREEN_HEIGHT - 25); tft.print("Status:");
        tft.setCursor(5, SCREEN_HEIGHT - 12); tft.print("IP:");

        // Display Status text on the "Status" line
        int statusX = 5 + 55; int statusY = SCREEN_HEIGHT - 25;
        ...
        int titleX = 5 + 55; int titleY = 45;
        int titleWidth = SCREEN_WIDTH - titleX - 5; int titleHeight = 35;
        int charWidth = 6; int charsPerLine = titleWidth / charWidth;
        clearTextArea(titleX, titleY, titleWidth, titleHeight);
        tft.setTextColor(TFT_ORANGE, TFT_BLACK);
        tft.setCursor(titleX, titleY);
        String titleToDisplay = currentStreamTitle; titleToDisplay.trim(); // Trim initial title once
        if (titleToDisplay.length() > 0) {
             ...
             while (titleToDisplay.length() > 0 && currentLine < maxLines) {
                 ...
                 if (titleToDisplay.length() <= charsPerLine) {
                     ...
                     for (int i = charsPerLine -1; i >=0; i--) { // Find last space within limit
                          if (titleToDisplay.charAt(i) == ' ') {
                              breakPoint = i;
                              break;
                          }
                     }
                     if (breakPoint != -1) { // Found a space to break at
                         ...
                         // *** END CORRECTION ***
                     } else { // No space found, just break at char limit
                 ...
                 currentLine++;
                 if (titleToDisplay.length() > 0 && currentLine < maxLines) { // If more text and lines available
                      tft.setCursor(titleX, titleY + (currentLine * 12)); // Move cursor for next line
                 }
             }
        } else { tft.print("---"); } // Show placeholder if no title
...
void updateSmallIndicator() {
    if (currentAudioMode != MODE_WIFI_RADIO || showingAnimation || isSwitchingMode) {
       if (!needsStaticIndicatorDraw) { tft.fillRect(INDICATOR_X, INDICATOR_Y, INDICATOR_SIZE, INDICATOR_SIZE, TFT_BLACK); } // Clear if it was drawn before
    ...
    } else if (isBufferingWarningActive) { // Check buffering flag
    ...
    if (needsStaticIndicatorDraw || (shouldPulse && (currentTime - lastIndicatorUpdateTime > INDICATOR_PULSE_INTERVAL))) {
        ...
        switch (currentState) {
            ...
            case STATE_BUFFERING_WARN:   indicatorColor = TFT_YELLOW; break; // Yellow for warning
            case STATE_STOPPED_OK:
            default:                     indicatorColor = TFT_RED; break;
        }
...
void loop() {
    ...
    if (currentAudioMode == MODE_WIFI_RADIO) {
        ...
        if (WiFi.status() == WL_CONNECTED) { audio.loop(); } // Only loop audio if connected

        if (visualizerModeActive && showingAnimation) {
            runVisualizer(); // Run visualizer if active
        } else {
            // --- Normal WiFi Radio Display Logic ---
            visualizerModeActive = false; showingAnimation = false; // Ensure flags are off
            ...
            if (WiFi.status() != WL_CONNECTED) {
                if (currentStatus != "WiFi Lost" && currentStatus != "No WiFi Cfg" && !currentStatus.startsWith("WiFi Fail") && !currentStatus.startsWith("WiFi Try")) {
                    ...
                    updateDisplay(); Serial.println("Loop: WiFi connection lost.");
                }
            }
            ...
            if (currentPlayState != playflag) {
                ...
                if (!playflag) {
                    ...
                    currentBitrate = ""; currentStreamTitle = ""; // Clear dynamic info
        ...
        if (playflag) {
            if (currentTime - lastLedToggle > 500) { // Blink interval
                lastLedToggle = currentTime;
                ledState = !ledState;
                digitalWrite(LED, ledState ? LOW : HIGH); // LED ON when LOW
            }
        } else {
            if (ledState) { // Ensure LED is OFF if stopped
                digitalWrite(LED, HIGH); ledState = false;
            }
        }
        ...
        if (bt_status == "Connected") {
            if (!ledState) { digitalWrite(LED, LOW); ledState = true;}
        } else {
            if (ledState) { digitalWrite(LED, HIGH); ledState = false; }
        }
    }
...
// =========================================================================
void handlePlayStopTap(Button2& btn) {
    if (currentAudioMode != MODE_WIFI_RADIO || isSwitchingMode) return;
    Serial.println("Tap: Play/Stop Button (Radio)");
    if (!playflag) { // If currently stopped
        if (WiFi.status() == WL_CONNECTED && !arrayURL.empty()) {
            ...
            if (sflag >= arrayURL.size()) sflag = 0;
            char* currentURL = (char*)arrayURL[sflag].c_str();
            currentStationName = arrayStation[sflag];
            currentStreamTitle = ""; currentBitrate = "";
            if (!audio.connecttohost(currentURL)) {
                currentStatus = "Connect FAIL"; statusColor = TFT_RED; if (!showingAnimation) updateDisplay();
            } else {
                playflag = true;
            }
        } else if (arrayURL.empty()) {
            currentStatus = "No Stations"; statusColor = TFT_RED; if (!showingAnimation) updateDisplay();
        } else { // No WiFi
            currentStatus = "No WiFi"; statusColor = TFT_RED; if (!showingAnimation) updateDisplay();
        }
...
void enterConfigModeLongClick(Button2& btn) {
    ...
    tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
    tft.setTextSize(1);
    tft.setCursor(10, 50); tft.print("Rebooting to Config Mode...");
...
void handleNextStationTap(Button2& btn) {
    ...
    if (!playflag && !arrayStation.empty()) {
        ...
        currentStatus = "Radio Ready"; statusColor = TFT_YELLOW;
        currentStreamTitle = ""; currentBitrate = "";
...
void handleVisualizerCycle(Button2& btn) {
    ...
    if (!showingAnimation) {
        ...
        drawScreenLayout(); updateDisplay(); // Redraw standard UI
    }
}
...
void handlePotentiometer() {
    ...
    if (lastStablePotADC == -1 || abs(potValue - lastStablePotADC) > ADC_CHANGE_THRESHOLD) {
        if (currentAudioMode == MODE_WIFI_RADIO) {
            if (potValue < 50) { newVolumeMapped = 0; }
            else { newVolumeMapped = map(potValue, 50, 4095, 0, 21); }
            ...
            if (potValue < 50) { newVolumeMapped = 0; }
            else { newVolumeMapped = map(potValue, 50, 4095, 0, 127); }
...
// *** UPDATED loadDefaultStations function ***
void loadDefaultStations() {
    ...
    arrayStation.push_back("SomaFM Def Con"); arrayURL.push_back("http://ice1.somafm.com/defcon-128-mp3");
    arrayStation.push_back("Classic FM UK"); arrayURL.push_back("http://media-ice.musicradio.com/ClassicFMMP3");
    arrayStation.push_back("NPO Radio 2 NL"); arrayURL.push_back("http://icecast.omroep.nl/radio2-bb-mp3");
    arrayStation.push_back("Virgin Radio IT"); arrayURL.push_back("http://icecast.unitedradio.it/Virgin.mp3"); // ADDED
    arrayStation.push_back("Naxos Classical"); arrayURL.push_back("http://naxos.cdnstream.com:80/1255_128"); // ADDED
...
bool loadConfiguration() {
    ...
    if (SPIFFS.exists(STATIONS_PATH)) {
        ...
        if (!file) {
            ...
            if (error) {
                Serial.print("ERROR: deserializeJson() failed: "); Serial.println(error.c_str());
            } else {
                arrayURL.clear(); arrayStation.clear();
                ...
                for (JsonObject stationObj : stationsArray) {
                    if (stationObj.containsKey("name") && stationObj.containsKey("url")) {
                        ...
                        } else { Serial.println("Skipping invalid station entry in JSON."); }
                    } else { Serial.println("Skipping incomplete station entry in JSON."); }
                }
    ...
    } else { Serial.println("Stations file not found (" + String(STATIONS_PATH) + ")."); }
    ...
    if (arrayStation.empty()) {
        ...
        if (sflag >= arrayStation.size()) sflag = 0;
        currentStationName = arrayStation[sflag];
    }
...
bool saveStationsToSPIFFS() {
    ...
    for (size_t i = 0; i < arrayURL.size(); ++i) {
        ...
        stationObj["name"] = arrayStation[i]; stationObj["url"] = arrayURL[i];
    }
    ...
    file.close();
    if (bytesWritten == 0 && arrayURL.size() > 0) { Serial.println("ERROR: Failed to write stations data to file (wrote 0 bytes)."); return false; }
...
void handleRoot() {
    ...
    preferences.begin("wifi-creds", true); String current_ssid = preferences.getString("ssid", ""); preferences.end();
    ...
    int n = WiFi.scanNetworks(); Serial.printf("Scan finished inside handleRoot, %d networks found.\n", n);
    if (n > 0) {
        for (int i = 0; i < n; ++i) {
            String scanned_ssid = WiFi.SSID(i); String selected_attr = (scanned_ssid == current_ssid) ? " selected" : ""; String security_type;
            switch(WiFi.encryptionType(i)) {
                case WIFI_AUTH_OPEN: security_type = "Open"; break; case WIFI_AUTH_WEP: security_type = "WEP"; break;
                case WIFI_AUTH_WPA_PSK: security_type = "WPA-PSK"; break; case WIFI_AUTH_WPA2_PSK: security_type = "WPA2-PSK"; break;
                case WIFI_AUTH_WPA_WPA2_PSK: security_type = "WPA/WPA2-PSK"; break; case WIFI_AUTH_WPA2_ENTERPRISE: security_type = "WPA2-Ent"; break;
                case WIFI_AUTH_WPA3_PSK: security_type = "WPA3-PSK"; break; case WIFI_AUTH_WPA2_WPA3_PSK: security_type = "WPA2/WPA3-PSK"; break;
                default: security_type = "Unknown"; break;
            }
            html += "<option value='" + html_escape(scanned_ssid) + "'" + selected_attr + ">";
            html += html_escape(scanned_ssid) + " (" + WiFi.RSSI(i) + " dBm, " + security_type + ")"; html += "</option>";
        }
    } else { html += "<option value=''>No networks found</option>"; }
    WiFi.scanDelete();
    html += "</select><br>";
    ...
    for (size_t i = 0; i < arrayStation.size(); ++i) {
        ...
        html += "<input type='hidden' name='url" + String(i) + "' value='" + html_escape(arrayURL[i]) + "'>"; html += "</li>";
    }
    html += "</ul>";
    ...
    html += "<hr>"; html += "<button type='submit'>Save Station List & Reboot</button></form></div>";
    html += "<script>";
...
void handleSaveWifi() {
    ...
    if (server.hasArg("ssid") && server.hasArg("password")) {
        String ssid = server.arg("ssid"); String password = server.arg("password");
        if (ssid.length() == 0) { Serial.println("ERROR: Received empty SSID."); server.send(400, "text/plain", "Bad Request: SSID cannot be empty."); return; }
        Serial.println("Received SSID: " + ssid); Serial.println("Received Password: [REDACTED]");
        preferences.begin("wifi-creds", false); preferences.putString("ssid", ssid); preferences.putString("password", password); preferences.end();
    ...
    } else { Serial.println("ERROR: Missing ssid or password in POST request."); server.send(400, "text/plain", "Bad Request: Missing parameters."); }
}
...
void handleSaveStations() {
    ...
    arrayURL.clear(); arrayStation.clear();
    for (int i = 0; ; i++) {
        String nameArgName = "name" + String(i); String urlArgName = "url" + String(i);
        if (server.hasArg(nameArgName) && server.hasArg(urlArgName)) {
            String name = server.arg(nameArgName); String url = server.arg(urlArgName);
            if (name.length() > 0 && url.length() > 0 && url.startsWith("http://")) {
                arrayStation.push_back(name); arrayURL.push_back(url); Serial.println("  Adding: \"" + name + "\" -> " + url);
            } else { Serial.println("  Skipping invalid entry #" + String(i) + " (empty or bad URL format)."); }
        } else { break; }
    }
    ...
    if (saveStationsToSPIFFS()) {
        ...
        html += "<script>var count = 3; function countdown(){ if(count>0) document.getElementById('countdown').innerText=count; count--; } setInterval(countdown, 1000);</script>";
        html += "</body></html>"; server.send(200, "text/html", html);
    } else { Serial.println("ERROR: Failed to save stations to SPIFFS!"); server.send(500, "text/plain", "Internal Server Error: Could not save stations."); }
}
...
void handleReboot() {
    ...
    server.send(200, "text/html", html); delay(500); ESP.restart();
}

void handleNotFound() {
    Serial.println("Serving 404 Not Found"); server.send(404, "text/plain", "404: Not Found");
}
...
void connectToWiFi() {
    ...
    if (sta_ssid.length() == 0) {
        Serial.println("WiFi credentials not set in NVS."); Serial.println("Use TTGO Top Button (GPIO 35) long press for Config Mode.");
        currentStatus = "No WiFi Cfg"; statusColor = TFT_RED; if (!showingAnimation) updateDisplay(); return;
    }
    ...
    while (retryCount < maxRetries) {
        ...
        currentStatus = "WiFi Try " + String(retryCount + 1); statusColor = TFT_ORANGE; if (!showingAnimation) updateDisplay();
        ...
        while (WiFi.status() != WL_CONNECTED) {
            ...
            Serial.print(".");
            if (!isSwitchingMode) { buttonBacklight.loop(); buttonConfig.loop(); buttonPlayStop.loop(); buttonNext.loop(); }
            if (!showingAnimation && currentAudioMode == MODE_WIFI_RADIO) updateSmallIndicator();
            delay(500);
        }
        if (WiFi.status() == WL_CONNECTED) {
            Serial.println("\nWiFi connected!"); Serial.print("IP Address: "); Serial.println(WiFi.localIP());
            currentStatus = "Connected"; statusColor = TFT_GREEN; if (!showingAnimation) updateDisplay(); return;
        } else {
            Serial.println("\n -> WiFi Connection Failed."); WiFi.disconnect(true); delay(1000); retryCount++;
            currentStatus = "WiFi Fail " + String(retryCount); statusColor = TFT_RED; if (!showingAnimation) updateDisplay();
        }
    }
    Serial.println("-----------------------------------------"); Serial.println("WiFi connection failed after all retries."); Serial.println("-----------------------------------------");
    currentStatus = "WiFi Failed!"; statusColor = TFT_RED; if (!showingAnimation) updateDisplay();
}
...
// =========================================================================
void audio_info(const char *info){
    if(currentAudioMode != MODE_WIFI_RADIO) return;
    Serial.print("Info:        "); Serial.println(info);
    String infoStr = String(info); infoStr.toLowerCase();

    if (infoStr.indexOf("error") != -1 || infoStr.indexOf("failed") != -1) {
        if(currentStatus != "Connect FAIL" && currentStatus != "WiFi Failed!") {
             currentStatus = "Stream Err"; statusColor = TFT_RED;
        ...
        if(currentStatus != "Playing") {
             currentStatus = "Playing"; statusColor = TFT_GREEN;
             if (!showingAnimation) updateDisplay();
        }
        ...
        if (currentStatus != "Playing") {
             currentStatus = "Syncing..."; statusColor = TFT_CYAN;
             if (!showingAnimation) updateDisplay();
        }
...
void audio_id3data(const char *info){
    ...
    if (currentStreamTitle.length() == 0 && strlen(info) > 3) {
        currentStreamTitle = String(info); if (!showingAnimation) updateDisplay();
    }
}
void audio_eof_mp3(const char *info){
    if(currentAudioMode != MODE_WIFI_RADIO) return;
    ...
    playflag = false; currentStreamTitle = ""; currentBitrate = "";
...
void audio_showstation(const char *info){
    ...
    if (strlen(info) > 0) {
        ...
        if (icyName != currentStationName) { Serial.printf(" (ICY Name differs from list: %s)\n", currentStationName.c_str()); }
    } else {
        if (!arrayStation.empty() && sflag < arrayStation.size()) { currentStationName = arrayStation[sflag]; }
        else { currentStationName = "Unknown"; }
    }
    if (!showingAnimation) updateDisplay();
}
void audio_showstreamtitle(const char *info){
    ...
    String newTitle = String(info); newTitle.trim();
    if (newTitle != currentStreamTitle && newTitle.length() > 0) {
        currentStreamTitle = newTitle; if (!showingAnimation) updateDisplay();
    } else if (newTitle.length() == 0 && currentStreamTitle.length() > 0) {
        currentStreamTitle = ""; if (!showingAnimation) updateDisplay();
    }
}
void audio_bitrate(const char *info){
    if(currentAudioMode != MODE_WIFI_RADIO) return;
...
void connection_state_changed(esp_a2d_connection_state_t state, void *ptr) {
    ...
    switch (state) {
        case ESP_A2D_CONNECTION_STATE_DISCONNECTED:
            bt_status = "Disconnected"; bt_peer_name = ""; currentStreamTitle = ""; btAudioLevel = 0; updateNeeded = true; Serial.println("A2DP Disconnected"); break;
        case ESP_A2D_CONNECTION_STATE_CONNECTING:
            bt_status = "Connecting..."; updateNeeded = true; Serial.println("A2DP Connecting..."); break;
        case ESP_A2D_CONNECTION_STATE_CONNECTED:
            bt_status = "Connected"; bt_peer_name = a2dp_sink.get_peer_name();
            if(bt_peer_name.isEmpty()) {
                esp_bd_addr_t* p_last_addr = a2dp_sink.get_last_peer_address();
                if (p_last_addr != nullptr) {
                     char addr_str[18]; snprintf(addr_str, sizeof(addr_str), "%02X:%02X:%02X:%02X:%02X:%02X", (*p_last_addr)[0], (*p_last_addr)[1], (*p_last_addr)[2], (*p_last_addr)[3], (*p_last_addr)[4], (*p_last_addr)[5]);
                     bt_peer_name = String(addr_str);
                } else { bt_peer_name = "Unknown Device"; Serial.println("Could not retrieve peer address."); }
            }
            updateNeeded = true; Serial.printf("A2DP Connected to: %s\n", bt_peer_name.c_str()); a2dp_sink.set_volume(currentVolume); Serial.printf(" -> Set BT volume to %d on connect\n", currentVolume); break;
        case ESP_A2D_CONNECTION_STATE_DISCONNECTING:
            bt_status = "Disconnecting"; btAudioLevel = 0; updateNeeded = true; Serial.println("A2DP Disconnecting"); break;
        default:
            bt_status = "Unknown State"; updateNeeded = true; Serial.printf("A2DP Unknown Connection State: %d\n", state); break;
    }
    if (updateNeeded && !showingAnimation && !isSwitchingMode) { updateDisplay(); }
}
...
void audio_state_changed(esp_a2d_audio_state_t state, void *ptr) {
    ...
    bool updateNeeded = false;
     switch (state) {
         case ESP_A2D_AUDIO_STATE_STARTED: Serial.println("A2DP Audio Started"); break;
         case ESP_A2D_AUDIO_STATE_STOPPED: Serial.println("A2DP Audio Stopped"); btAudioLevel = 0; break; // Reset level viz
         case ESP_A2D_AUDIO_STATE_REMOTE_SUSPEND: Serial.println("A2DP Audio Suspended"); bt_status = "Suspended"; btAudioLevel = 0; updateNeeded = true; break;
         default: Serial.printf("A2DP Unknown Audio State: %d\n", state); break;
     }
    if (updateNeeded && !showingAnimation && !isSwitchingMode) { updateDisplay(); }
}
...
void avrc_metadata_callback(uint8_t data_type, const uint8_t *data) {
    ...
    if (data_type == ESP_AVRC_MD_ATTR_TITLE) {
        String newTitle = String((char*)data); newTitle.trim();
        if (newTitle != currentStreamTitle && newTitle.length() > 0) {
            currentStreamTitle = newTitle; Serial.printf(" -> Track Title Updated: %s\n", currentStreamTitle.c_str());
...
void audio_data_callback(const uint8_t *data, uint32_t len) {
    ...
    for (int i = 0; i < samples_received; i++) {
        ...
        if (sample_count % RMS_WINDOW_SIZE == 0) {
            ...
            double mean_sq = sum_sq / RMS_WINDOW_SIZE; double rms = sqrt(mean_sq);
